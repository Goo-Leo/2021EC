...
uint8_t SDRAM_Test(void)
{
    ...
    for(i = 0; i < SDRAM_Size/2;i++ )
    {
        ...
        if( ReadData != (uint16_t)i )      //�?测数据，若不相等，跳出函�?,返回�?测失败结果�??
        {
            printf("SDRAM测试失败！！\r\n");
            return ERROR;	 // 返回失败标志
        }
    ...
    printf("16位数据宽度读写�?�过，以8位数据宽度写入数据\r\n");
    for (i = 0; i < 255; i++)
    {
    ...
    for (i = 0; i < 255; i++)
    {
        ...
        if( ReadData_8b != (uint8_t)i )      //�?测数据，若不相等，跳出函�?,返回�?测失败结果�??
        {
            printf("8位数据宽度读写测试失败！！\r\n");
...
void SDRAM_Initialization_Sequence(SDRAM_HandleTypeDef *hsdram, FMC_SDRAM_CommandTypeDef *Command)
{
    ...
    Command->CommandMode 					= FMC_SDRAM_CMD_CLK_ENABLE;	// �?启SDRAM时钟
    Command->CommandTarget 				= FMC_COMMAND_TARGET_BANK; 	// 选择要控制的区域
    Command->AutoRefreshNumber 			= 1;
    Command->ModeRegisterDefinition 	= 0;

    HAL_SDRAM_SendCommand(hsdram, Command, SDRAM_TIMEOUT);	// 发�?�控制指�?
    SDRAM_delay(1);		// 延时等待
    ...
    hsdram->Instance->SDRTR |= ((uint32_t)((1386)<< 1));	// 设置刷新计数�?
}
/* USER CODE END 0 */
